      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 bg-surface border border-outline-variant rounded-xl shadow-sm overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className="w-full px-4 py-2.5 border border-outline-variant rounded-lg font-body-md text-on-surface bg-surface-container-lowest focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all cursor-pointer"
                >
            ...
            {/* File Upload (UI mock only for now, can implement later with presigned URLs) */}
            <button
              type="button"
            ...
            >
              ...
              <p className="font-label-md text-primary">{selectedFileName ? selectedFileName : "Click to upload screenshots (Optional)"}</p>
                ...
                initialTickets.map((ticket) => (
                  <div key={ticket.id} onClick={() => router.push(`/dashboard/search?q=${encodeURIComponent(ticket.subject)}`)} className="p-4 hover:bg-surface-container-lowest transition-colors cursor-pointer group">
                    <div className="flex justify-between items-start mb-2">
                      <span className="font-label-sm font-mono text-on-surface-variant group-hover:text-secondary">#{ticket.id.substring(ticket.id.length - 6).toUpperCase()}</span>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold ${
                          ticket.status === "RESOLVED" ? "bg-primary-container text-primary" : 
                          ticket.status === "IN_PROGRESS" ? "bg-secondary-container text-on-secondary-container" : 
                    ...
                    <p className="font-body-sm text-on-surface-variant">Updated {formatDate(ticket.updatedAt)}</p>
                  </div>
                ))
              )}
            </div>

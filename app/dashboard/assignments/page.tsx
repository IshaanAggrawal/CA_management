...
const initialAssignments: Assignment[] = [
  {
    id: 1,
    title: "GST Audit - Q3 2023",
    client: "Global Logistics Pvt Ltd.",
    team: ["SM", "DR"],
    status: "In Progress",
  ...
  {
    id: 2,
    title: "ITR Filing - FY 22-23",
    client: "Apex Solutions Inc.",
    team: ["JW", "DR"],
    status: "Review",
  ...
  },
  {
    ...
    team: ["MK"],
    status: "Completed",
    statusColor: "bg-on-primary-fixed-variant text-secondary-fixed",
    dueDate: "Oct 15, 2023",
    timeLeft: "Completed",
    progress: 100,
  }
...
export default function AssignmentsPage() {
  ...
  const columns: { title: string; status: Assignment["status"]; border: string; bg: string }[] = [
    { title: "Not Started", status: "Not Started", border: "border-t-slate-400", bg: "bg-slate-50/50" },
    { title: "In Progress", status: "In Progress", border: "border-t-blue-500", bg: "bg-blue-50/10" },
  ...
  const handleDrop = (e: React.DragEvent, status: Assignment["status"]) => {
    e.preventDefault();
    if (draggedTaskId === null) return;
    
    setAssignments(prev => prev.map(task => {
      if (task.id === draggedTaskId) {
        // Update color and progress logic based on new status
        let newColor = task.statusColor;
        let newProgress = task.progress;
        
        switch (status) {
          case "Not Started": newColor = "bg-surface-container-high text-on-surface-variant"; newProgress = 0; break;
          case "In Progress": newColor = "bg-secondary-container text-on-secondary-container"; newProgress = 50; break;
          case "Review": newColor = "bg-tertiary-fixed text-on-tertiary-fixed-variant"; newProgress = 90; break;
          case "Completed": newColor = "bg-on-primary-fixed-variant text-secondary-fixed"; newProgress = 100; break;
        }
        
        return { ...task, status, statusColor: newColor, progress: newProgress };
      }
      return task;
    }));
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <button onClick={() => alert("Exporting assignments to CSV...")} className="flex items-center gap-2 px-4 py-2 border border-outline-variant rounded-lg text-on-surface-variant font-label-md text-label-md hover:bg-surface-container transition-colors cursor-pointer">
                  ...
                  <tr key={task.id} className="hover:bg-surface-container-low transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        ...
                        <span className="font-body-sm text-body-sm text-on-surface-variant">{task.client}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center -space-x-2">
                        {task.team.map((initial, idx) => (
                          <div
                            key={idx}
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-label-md font-semibold ${task.statusColor}`}>
                        {task.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-body-md text-body-md text-on-surface-variant">{task.dueDate}</td>
                    <td className="px-6 py-4">
                      {task.status === "Completed" ? (
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`font-body-md text-body-md font-bold ${task.timeLeft === "Overdue" ? "text-error" : "text-on-surface"}`}>
                            {task.timeLeft}
                          </span>
                          <div className="w-16 h-1 bg-surface-container rounded-full overflow-hidden">
                            <div className={`h-full ${task.timeLeft === "Overdue" ? "bg-error" : "bg-primary"}`} style={{ width: `${task.progress}%` }}></div>
                          </div>
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => alert("Opening task actions menu...")} className="text-on-surface-variant hover:text-primary transition-colors cursor-pointer">
                  ...
                  {tasksInColumn.length === 0 ? (
                    ...
                    tasksInColumn.map((task) => (
                      <div
                        ...
                        className={`bg-white border border-outline-variant/60 rounded-xl p-4 shadow-sm hover:shadow-md hover:border-primary/40 transition-all group flex flex-col justify-between cursor-grab active:cursor-grabbing ${draggedTaskId === task.id ? 'opacity-50 border-dashed scale-95' : ''}`}
                      >
                        <div>
                          <div className="flex justify-between items-start gap-2 mb-2">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold ${task.statusColor}`}>
                              {task.status}
                            </span>
                            <button onClick={() => alert("Opening task details...")} className="text-on-surface-variant opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                          ...
                          <p className="text-body-sm text-on-surface-variant mb-4">{task.client}</p>
                        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-primary text-on-primary rounded-xl p-6 shadow-lg relative overflow-hidden">
          <div className="relative z-10">
            ...
            <button onClick={() => alert("Opening review portal...")} className="w-full mt-6 py-2 border border-secondary text-secondary font-label-md text-label-md rounded hover:bg-secondary hover:text-on-secondary transition-all cursor-pointer">
              Open Review Portal
            </button>
          </div>

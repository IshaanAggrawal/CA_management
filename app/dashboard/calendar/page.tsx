...
// Mock events
const MOCK_EVENTS = [
  { id: 1, date: 4, type: "income", title: "ITR-4 Filing (12)", group: "Income Tax" },
  { id: 2, date: 7, type: "gst", title: "GSTR-1 Monthly (48)", group: "GST Filing" },
  { id: 3, date: 7, type: "tds", title: "TDS Deposit (05)", group: "TDS / TCS" },
  { id: 4, date: 15, type: "roc", title: "AOC-4 (08)", group: "ROC Compliance" },
  { id: 5, date: 21, type: "gst", title: "GSTR-3B (22)", group: "GST Filing" },
  { id: 6, date: 30, type: "income", title: "Quarterly Filing (14)", group: "Income Tax" },
];

export default function CalendarPage() {
  const router = useRouter();
  const [currentDate, setCurrentDate] = useState(new Date(2024, 9, 1)); // Starts at Oct 2024
  ...
  const renderDays = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      const isToday = i === 7 && month === 9 && year === 2024; // Mock today as Oct 7, 2024
        ...
        >
          ...
          <div className="mt-1.5 space-y-1.5 overflow-y-auto max-h-[calc(100%-24px)] custom-scrollbar pr-1">
            {dayEvents.map(event => (
              <div 
                key={event.id} 
                onClick={(e) => { e.stopPropagation(); alert(`Opening details for ${event.title}`); }}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={() => alert("Opening calendar filters...")} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg font-semibold text-xs text-slate-700 hover:bg-slate-50 transition-colors shadow-sm cursor-pointer">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs px-2 py-1 bg-slate-100 text-slate-600 font-bold rounded-lg border border-slate-200">
                {selectedDate ? MOCK_EVENTS.filter(e => e.date === selectedDate).length : 0} Filings
              </span>
            </div>
            
            {selectedDate && MOCK_EVENTS.filter(e => e.date === selectedDate).length > 0 ? (
              <div className="space-y-4 max-h-[350px] overflow-y-auto custom-scrollbar pr-2">
                {MOCK_EVENTS.filter(e => e.date === selectedDate).map(event => (
                  <div key={event.id}>
                    ...
                    <div className="space-y-2">
                      <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 flex items-center justify-between hover:border-slate-300 transition-colors cursor-pointer">
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 mt-0.5">ID: 27AAAAA0000A1Z5</p>
                        </div>
                        <span className="text-[10px] px-2 py-1 bg-amber-100 text-amber-700 rounded font-bold uppercase tracking-wider">Pending</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-3">
                <div className="p-4 bg-black/20 rounded-lg border border-white/10 backdrop-blur-sm">
                  ...
                  <div className="flex justify-between items-center text-[11px] font-bold">
                    ...
                    <button onClick={() => alert("Loading SMS Logs...")} className="text-white underline hover:text-teal-100 cursor-pointer">View Logs</button>
                  </div>
                </div>
              </div>
              <button onClick={() => alert("Opening automation settings...")} className="w-full mt-4 py-2.5 bg-white/10 rounded-lg text-white font-bold text-sm hover:bg-white/20 transition-colors border border-white/20 cursor-pointer shadow-sm">
                Configure Reminders
              </button>
            </div>
            {/* Decorative element */}

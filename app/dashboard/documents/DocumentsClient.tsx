...
export default function DocumentsClient({ documents = [] }: any) {
  return (
    <>
      {/* Action Header */}
      ...
      <div className="bg-white border border-outline-variant rounded-xl overflow-hidden shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant">
              {documents.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-10 text-center text-on-surface-variant font-label-md">No documents found</td>
                </tr>
              ) : (
                documents.map((doc: any) => (
                  <tr key={doc.id} className="hover:bg-surface-container-lowest transition-colors cursor-pointer">
                    ...
                    <td className="px-6 py-4 font-body-md text-body-md text-on-surface-variant">Uncategorized</td>
                    <td className="px-6 py-4 font-body-md text-body-md text-on-surface-variant">{new Date(doc.created_at).toLocaleDateString()}</td>
                    <td className="px-6 py-4 font-body-md text-body-md text-on-surface-variant">{(doc.metadata?.size / 1024).toFixed(1)} KB</td>
                    <td className="px-6 py-4">
        ...
        <div className="px-6 py-4 border-t border-outline-variant flex justify-between items-center bg-surface-container-low">
          <span className="font-label-sm text-label-sm text-on-surface-variant">Showing {documents.length} of {documents.length} files</span>
          <div className="flex gap-4">

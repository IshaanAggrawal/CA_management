...
const initialClients: Client[] = [
  {
    ...
    partner: "S. Sharma",
    type: "Corporate",
    panGstin: "ABCDE1234F / 27ABCDE1234F1Z5",
    contactPerson: "Rahul Kapoor",
    email: "r.kapoor@acmecorp.com",
    status: "Active",
  },
  {
    id: 2,
  ...
  {
    id: 3,
    name: "TechVanguard LLP",
    partner: "R. Malhotra",
    type: "LLP",
    panGstin: "KLMN09812P / 27KLMN09812P2Z8",
    contactPerson: "Sneha Verma",
    email: "contact@techvanguard.in",
    status: "Active",
  },
  {
    id: 4,
    ...
    partner: "S. Sharma",
    type: "Corporate",
    panGstin: "QRSTU3456V / 09QRSTU3456V1A7",
    contactPerson: "Anita Iyer",
    email: "aiyer@horizon.com",
    status: "Inactive",
  },
];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                ...
                <tr key={client.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-slate-400 font-medium">Partner: {client.partner}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 font-medium">{client.type}</td>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 bg-slate-50/50">
          <p className="text-sm font-semibold text-slate-500">
            Showing <span className="text-slate-900 font-bold">1</span> to <span className="text-slate-900 font-bold">5</span> of <span className="text-slate-900 font-bold">1,248</span> clients
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#1e293b] text-white rounded-xl p-6 shadow-md flex flex-col justify-between relative overflow-hidden">
          ...
          <button onClick={() => alert("Loading Audit AI Engine...")} className="w-full mt-6 py-2.5 bg-white text-slate-900 font-semibold text-sm rounded-lg hover:bg-slate-50 transition-all cursor-pointer shadow-sm relative z-10 text-center">
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col">
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-1.5">
                ...
                <input type="text" className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-[#005c53]" placeholder="e.g. Acme Corp Ltd" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Entity Type</label>
                  <select className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-[#005c53]">
                    <option>Corporate</option>
                    <option>Individual</option>
                    <option>LLP</option>
                  </select>
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">PAN / GSTIN</label>
                  <input type="text" className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-[#005c53]" placeholder="ABCDE1234F" />
                </div>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 font-bold text-slate-600 hover:bg-slate-200 rounded-lg transition-colors cursor-pointer">Cancel</button>
              <button onClick={() => { alert("Client added successfully!"); setIsModalOpen(false); }} className="px-6 py-2 bg-[#005c53] hover:bg-[#004841] text-white font-bold rounded-lg shadow-sm transition-colors cursor-pointer">Save Client</button>
            </div>
          </div>
        </div>
      )}
